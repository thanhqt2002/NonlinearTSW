...
from tsw import generate_trees_frames
import cfg
args = cfg.parse_args()
...
modes = ['linear', 'linear', 'linear', 'linear', 'linear', 'linear', 'linear', 'linear', 'linear', 'linear']
# titles = ['SW', 'C-TSW', 'CC-TSW', 'Pow-TSW', 'TSW-SL-distance-based', 'TSW-SL-uniform', 'TSW-SL-orthogonal']
titles = ['SW', 'Circular-GTSW', 'CC-TSW', 'Pow-TSW', 'TSW-SL-distance-based', 'TSW-SL-uniform', 'TSW-SL-orthogonal', 'LCV-SW', 'SWGG', 'Max-SW']
# titles = ['Circular-GTSW']
...
lear_rates = [args.lr_sw, args.lr_tsw_sl, args.lr_tsw_sl, args.lr_tsw_sl, args.lr_tsw_sl, args.lr_tsw_sl, args.lr_tsw_sl, args.lr_sw, args.lr_sw, args.lr_sw, args.lr_sw]
n_projs = [args.L, int(args.L / args.n_lines), int(args.L / args.n_lines), int(args.L / args.n_lines), int(args.L / args.n_lines), int(args.L / args.n_lines), int(args.L / args.n_lines), args.L, args.L, args.L, args.L]
...
for k, title in enumerate(titles):
    for i, seed in enumerate(seeds):
        ...
        results_folder = f"./Results/Gradient_Flow_{folder_info}/seed{seed}"
        ...
        for t in tqdm(range(nofiterations)):
            ...
            if title == 'SW':
                ...
                loss += gradient_flow.TWD(X=X.to(device), Y=Y, ftype='circular', radius=0.01, theta=theta_twd, intercept=intercept_twd, mass_division='distance_based', p=args.p, delta=args.delta)
                end_time = time.time()  # End timing
                # print(f"Time taken for C-TWD: {end_time - start_time:.4f} seconds")
                ...
                loss += gradient_flow.TWD(X=X.to(device), Y=Y, ftype='circular_r0', theta=theta_twd, intercept=intercept_twd, mass_division='distance_based', p=args.p, delta=args.delta)
                end_time = time.time()  # End timing
                # print(f"Time taken for CC-TWD: {end_time - start_time:.4f} seconds")
                ...
                loss += gradient_flow.TWD(X=X.to(device), Y=Y, ftype='pow', degree=3, pow_beta=1, theta=theta_twd, intercept=intercept_twd, mass_division='distance_based', p=args.p, delta=args.delta)
                end_time = time.time()  # End timing
                # print(f"Time taken for Pow-TWD distance based: {end_time - start_time:.4f} seconds")
