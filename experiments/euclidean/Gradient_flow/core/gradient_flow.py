...
def TWD(X, Y, theta, intercept, mass_division = 'distance_based', p = 2, delta = 2., ftype='linear', degree=3, pow_beta=1, radius=2, device = 'cuda',):
    # print(p)
    # print(delta)
    # exit()
    L = theta.shape[0]
    nlines = theta.shape[1]
    TWD_obj = TSW(ntrees=L, nlines=nlines, p=p, delta=delta, mass_division=mass_division, ftype=ftype, degree=degree, pow_beta=pow_beta, radius=radius, device=device)
