...
# titles = ['SW', 'C-TSW', 'CC-TSW', 'Poly-TSW', 'TSW-SL-distance-based', 'TSW-SL-uniform', 'TSW-SL-orthogonal']
titles = ['Poly-TSW']
    ...
    for i, seed in enumerate(seeds):
        ...
        for t in tqdm(range(nofiterations)):
            ...
            if title == 'SW':
                ...
                loss += gradient_flow.TWD(X=X.to(device), Y=Y, ftype='pow', pow_beta=10, theta=theta_twd, intercept=intercept_twd, mass_division='distance_based', p=args.p, delta=args.delta)
                end_time = time.time()  # End timing
                # print(f"Time taken for Poly-TWD distance based: {end_time - start_time:.4f} seconds")
