...
def train(rank, args):
    ...
    if args.loss == 'cltwd':
        ...
        TWD_obj = torch.compile(TSW(ntrees=args.T, nlines=args.L, d=d, p=2, delta=args.twd_delta, pow_beta=args.twd_pow_beta, mass_division=args.twd_mass_division, ftype=args.twd_ftype, radius=args.twd_radius, device=device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--twd_mass_division', type=str, 
                        choices=["distance_based", "uniform"], default="distance_based")
    parser.add_argument('--twd_ftype', type=str, 
                        choices=["linear", "circular", "pow", "circular_r0"], default="linear")
