...
class TWConcurrentLines():
    def __init__(self, 
        ...
        assert self.ftype in ['linear', 'poly', 'circular', 'augmented', 'pow', 'circular_r0'], \
            "Invalid ftype. Must be one of 'linear', 'poly', 'circular', 'augmented', 'pow', 'circular_r0'"
...
if __name__ == "__main__":
    ...
    TW_obj = torch.compile(TWConcurrentLines(ntrees=ntrees, mass_division='distance_based', ftype='circular_r0'))
    theta, intercept = generate_trees_frames(ntrees, nlines, dn, gen_mode="gaussian_orthogonal")
    X = torch.rand(N, dn).to("cuda")
    Y = torch.rand(M, dm).to("cuda")
    TW_obj(X, Y, theta, intercept)
